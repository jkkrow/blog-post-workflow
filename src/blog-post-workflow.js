...
const isJson = acceptHeader === "application/json";
  ...
  promiseArray.push(
    new Promise((resolve, reject) => {
      promiseRetry((retry, tryNumber) => {
        ...
        return isJson
          ? axios({ url: siteUrl, method: "get", headers })
              .then((response) => response.data)
              .catch(retry)
          : parser.parseURL(siteUrl).catch(retry);
      }, retryConfig).then(
        (data) => {
          if (!data.items) {
    ...
    results.forEach((result, index) => {
      if (result.status === "fulfilled") {
        ...
        if (result.reason && result.reason.message && result.reason.message.startsWith("Status code")) {
          ...
          core.error(
            `Looks like your website returned ${code}, There is nothing blog post workflow` +
              ` can do to fix it. Please check your website's RSS feed generation source code. Also double check the URL.`
          );
          if (code === `503`) {
            core.error(
              `If you are using Cloudflare or Akamai,  make sure that you have the user agent ` +
